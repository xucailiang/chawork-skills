---
name: webapp-testing
description: 用于通过 Playwright 与本地 Web 应用交互和测试的工具包。支持验证前端功能、调试 UI 行为、捕获浏览器截图和查看浏览器日志。
...
translated_at: '2026-05-25T12:07:09.309Z'
...
**始终先用 `--help` 运行脚本** 以查看用法。在尝试运行脚本并确认绝对需要定制方案之前，不要去读源码。这些脚本可能非常庞大，会污染你的上下文窗口。它们的存在意义是作为黑盒脚本被直接调用，而不是被纳入你的上下文窗口。
...
要创建一个自动化脚本，只需包含 Playwright 逻辑（服务器由脚本自动管理）：
```python
...
❌ **不要** 在动态应用上还未等待 `networkidle` 时就去检查 DOM
✅ **要** 在检查之前等待 `page.wait_for_load_state('networkidle')`

## 最佳实践

- **把内置脚本当作黑盒使用** - 要完成一项任务时，先考虑 `scripts/` 中是否有可用的脚本能提供帮助。这些脚本能可靠地处理常见的复杂工作流，而不会污染上下文窗口。用 `--help` 查看用法，然后直接调用。
...
- **examples/** - 展示常见模式的示例：
  ...
  - `console_logging.py` - 在自动化过程中捕获 console 日志
